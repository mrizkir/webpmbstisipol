...
<head>
    ...
    <title>Admin Profil lulusan PMB-STISIPOL Raja Haji</title>
                ...
                <ul>
                    ...
                    <li class="megamenu-2"><a href="#"><span>Lainnya</span> <i
                        ...
                        <ul class="mobile-megamenu">
                            ...
                            <li><a href="{{ route('admin.beasiswa') }}"><span>Beasiswa</span></a></li>
        ...
        <section id="features-2" class="features-2 section">
          ...
          <script>
            ...
            <div class="container section-title mt-5" data-aos="fade-up">
                <span class="description-title">Profil Lulusan</span>
            ...
            <div class="container" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-4" data-aos="fade-up" data-aos-delay="150">
                        <div class="feature-card {{ $profillulusan->background }}">
                            <div class="feature-icon">
                                <i class="bi bi-app-indicator"></i>
                            </div>
                            <h3> {{ $profillulusan->judul }}</h3>
                            <p>{{ $profillulusan->deskripsi }}
</p>
                            <ul class="feature-benefits">
                                <li><i class="bi bi-check-circle-fill"></i> {{ $profillulusan->list1 }}</li>
                                @if ( $profillulusan->list2 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list2}}</li>
                                @endif
                                @if ( $profillulusan->list3 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list3}}</li>
                                @endif
                                @if ( $profillulusan->list4 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list4}}</li>
                                @endif
                                @if ( $profillulusan->list5 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list5}}</li>
                                @endif
                                @if ( $profillulusan->list6 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list6}}</li>
                                @endif
                                @if ( $profillulusan->list7 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list7}}</li>
                                @endif
                                @if ( $profillulusan->list8 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list8}}</li>
                                @endif
                                @if ( $profillulusan->list9 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list9}}</li>
                                @endif
                                @if ( $profillulusan->list10 )
                                <li><i class="bi bi-check-circle-fill"></i>{{$profillulusan->list10}}</li>
                                @endif

                            </ul>
                            <div class="feature-image">
                                <img src="../assets/img/lainnya/profil_lulusan/{{ $profillulusan->img }}" alt="Dashboard Interface"
                                    class="img-fluid" loading="lazy">
                            </div>
                            <a href="{{ route('showupdateMegamenuProfilLulusan', $profillulusan->id) }}" class="btn-tambah">Update Konten</a>
                            <form action="{{ route('hapusMegamenuProfilLulusan', $profillulusan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
    @csrf
    @method('DELETE')
...
<a href="{{ route('editMegamenuProfilLulusanTambah') }}" class="btn-tambah">Tambah Konten</a>
            </div>
